    ...
    >
        ...
        <div class="block md:hidden mb-4 border-b border-gray-200">
            <nav class="flex space-x-4" aria-label="Tabs">
                <template x-for="tab in ['dashboard', 'my-orders','my-vouchers', 'my-account', 'create-reviews']" :key="tab">
                    <button
                        @click="switchTab(tab)"
        ...
        <div class="hidden md:block md:col-span-2">
            <ul class="flex flex-col text-sm font-medium shadow-md">
                ...
                <li>
                    <button 
                        :class="activeTab === 'create-reviews' ? 'text-white bg-red-600' : 'bg-gray-50 hover:bg-gray-100 text-gray-600'" 
                        class="inline-flex items-center px-4 py-3 w-full gap-2 font-medium"
                        @click="switchTab('create-reviews')"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        <div class="md:col-span-5 space-y-4" x-data="{ visitedTabs: { 'dashboard': true } }">
            ...
            <!-- Tab Create Reviews -->
            <div 
                x-show="activeTab === 'create-reviews'" 
                x-init="$watch('activeTab', val => { if (val === 'create-reviews') visitedTabs['create-reviews'] = true })" 
                x-cloak
            >
                <template x-if="visitedTabs['create-reviews']">
                    <livewire:pages.tabs.create-reviews />
                </template>
            </div>
